...
- `workcenter_no`/`_name` = `aps_item_routing_spec` khớp `item_id`+`routing_id`+`gsystem_routing_id` (routing cross-check; `gsystem_id` KHÔNG phải khóa join — là PK riêng mỗi bảng, chỉ dùng tie-break đại diện `min(proc_sno, gsystem_id)`), lấy `workcenter_id` → `aps_workcenter`.
