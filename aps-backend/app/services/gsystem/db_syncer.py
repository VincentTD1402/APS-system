...
from app.models import (
    BOM, Demand, Equipment, Item, ItemRoutingSpec,
    MpsPlan, WorkCenter, WorkOrder,
)
...
def sync_items(session: Session, records: list[dict[str, Any]]) -> dict[int, Item]:
    ...
    for rec in _sorted(records):
        ...
        item.unit_cd = rec.get("stkUnitCd")
        item.asset_type_cd = rec.get("assetTypeCd")
        item.lot_yn = rec.get("lotYn")
        item.material = rec.get("material")
        item.purchase_price_vat_yn = rec.get("purchasePriceVatYn")
...
def sync_mps_plan(session: Session, records: list[dict[str, Any]]) -> int:
    ...
    Keyed by G-System record `id`. item_id FK resolved by gsystem_id — left
    null when the referenced item isn't in APS yet (optional on MpsPlan so
    orphaned MPS lines still get stored for visibility).
    """
    if not records:
        return 0
...
def sync_item_routing(
    ...
    Sole source for ItemRoutingSpec — GET /pd/itemRoutingMng?itemId= is the
    only routing/process-step API APS calls; every field captures the raw
    response verbatim (see model docstring). Keyed by G-System record `id`
    (one row per proc step / procSno). jph (EA/HR) is derived from workTime
    (seconds/EA): jph = 3600 / workTime.
    """
    if not records:
        return 0

    # G-System names the workcenter field "oscustId"/"custNm" (not
    # "workcenterId"/"workcenterNm") on this endpoint — verified live.
    ...
    for rec in _sorted(records):
        ...
        ir.item_id = item.id
        ir.item_no = rec.get("itemNo")
        ir.item_name = rec.get("itemNm")
        ...
        ir.workcenter_name_raw = rec.get("custNm")
        ...
        ir.gsystem_routing_id = _to_int(rec.get("routingId"))
        ir.routing_no = rec.get("routingNo")
        ir.routing_name = rec.get("routingNm")
        ir.routing_type_cd = rec.get("routingTypeCd")
        ir.routing_group_cd = rec.get("routingGroupCd")

        work_time = _to_float(rec.get("workTime"))
        ir.work_time = work_time
        ir.jph = round(3600.0 / work_time, 2) if work_time else None
        ir.lead_time = _to_int(rec.get("leadTime"))

        ir.inspection_yn = rec.get("inspectionYn")
        ir.inspec_type = rec.get("inspecType")
        ir.inspec_period = rec.get("inspecPeriod")
        ir.sample_qty = _to_int(rec.get("sampleQty"))
        ir.work_ins_yn = rec.get("workInsYn")
        ir.stock_yn = rec.get("stockYn")
        ir.loss_rate = _to_float(rec.get("lossRate"))
        ir.tool_id = _to_int(rec.get("toolId"))
        ir.remark = rec.get("remark")

        ir.corp_id = _to_int(rec.get("corpId"))
        ir.corp_name = rec.get("corpNm")
        ir.biz_id = _to_int(rec.get("bizId"))
        ir.item_cls1 = rec.get("itemCls1")
        ir.reg_dt = _parse_datetime(rec.get("regDt"))
        ir.reg_user_id = _to_int(rec.get("regUserId"))
        ir.reg_user_name = rec.get("regUserNm")
...
def sync_all(session: Session, data: dict[str, list[dict[str, Any]]]) -> None:
    ...
    Expects keys produced by GSystemClient.fetch_all():
      items, workcenters, bom, prod_plans, calendar, stock, customers.
    Routing/process-step data is NOT part of this pending-batch flow — it's
    synced separately, per-item, via itemRoutingMng (see sync_item_routing /
    _enrich_mps_and_item_routing in sync_service.py), the only routing source.

    Dependency-safe order per phase-04 sync plan.
    Caller owns commit/rollback.
    """
    item_index     = sync_items(session, data.get("items", []))
    sync_workcenters(session, data.get("workcenters", []))
