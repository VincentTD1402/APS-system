...
def _enrich_mps_and_item_routing(client: "GSystemClient") -> tuple[int, int, int, int, int]:
    ...
    Flow:
      ...
      2. create_planned_work_orders_from_mps_plan → for MPS lines with
         status_cd=="notCreated" and no linked WorkOrder yet, create a local
    ...
    from app.services.gsystem.db_syncer import (
        create_planned_work_orders_from_mps_plan,
        sync_item_routing,
        sync_mps_plan,
        sync_work_orders,
    )
    ...
    with SessionLocal() as session:
        ...
        planned_work_order_total = create_planned_work_orders_from_mps_plan(session)
        session.commit()

        try:
            work_order_records = client.fetch_work_orders(settings.GSYSTEM_DEFAULT_PAREA_ID)
        except Exception as exc:
            logger.warning("fetch_work_orders failed for pareaId=%s: %s", settings.GSYSTEM_DEFAULT_PAREA_ID, exc)
            work_order_records = []
